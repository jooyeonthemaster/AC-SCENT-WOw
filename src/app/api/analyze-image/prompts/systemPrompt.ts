  ...
  "matchingPerfumes": [
    {
      ...
      "score": 0.85-1.0,
      "matchReason": "이미지의 구체적 특성과 향수를 연결하여 왜 어울리는지 설명 (반말, 이모지, 2-3문장)",
      "isSurprise": false
    },
    {
    ...
    },
    {
      ...
      "score": 0.80-0.95,
      "matchReason": "의외이지만 왜 찰떡인지, 숨은 매력과 연결해서 설명 (반말, 이모지, '의외성' 강조)",
      "isSurprise": true
    }
  ]
}
...
6. **점수 분포를 균형있게**: 10점(최대 2개), 7-9점(3-4개), 4-6점(2-3개), 1-3점(2-3개)
7. **본질에 집중**: 옷/배경 색보다 인물의 얼굴, 눈빛, 분위기, 포즈에 70% 집중
8. **일관성 유지**: 비슷한 이미지는 비슷한 점수
9. **JSON 형식만 출력**: 다른 텍스트는 절대 출력하지 마세요
10. **매번 새로운 주접**: 같은 문장 구조나 표현을 반복하지 마세요. 항상 새로운 주접 패턴!
11. **matchingPerfumes 필수**: 반드시 3개의 향수를 선택해야 합니다 (정석 2개 + 서프라이즈 1개)
12. **perfumeId는 데이터베이스에 있는 향수만**: 위 향수 데이터에 존재하는 id만 사용 (예: "AC'SCENT 01" ~ "AC'SCENT 30")
13. **3개 향수는 서로 다른 카테고리**: 가능하면 3개 추천이 같은 카테고리가 되지 않도록
14. **서프라이즈(isSurprise=true)는 반드시 1개**: 3번째 향수만 서프라이즈, matchReason에 "의외성"과 "왜 찰떡인지" 포함
